...
def softmax_loss(x, y):
    ...
    x_shift = x - np.max(x, axis=1, keepdims=True) # (N, C)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, x_hat, sample_mean, sample_std, sample_var, eps, gamma, beta)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1.0 / N) * gamma * ivar * (
        N * dout
        - np.sum(dout, axis=0)
        - (x - mean) * (ivar ** 2) * np.sum(dout * (x - mean), axis=0)
    )
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    x_padded = np.pad(x, pad_width=((0, 0), (0, 0), (pad, pad), (pad, pad)), mode='constant', constant_values=0) # (N, C, H + 2 * pad, W + 2 * pad)
    N, C, H, W = x.shape
    F, _, HH, WW = w.shape
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    grad = dout[n, c, i, j]
                    dx[n, c, h_start+ind[0], w_start + ind[1]] += grad
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x_flat = x.transpose(0, 2, 3, 1).reshape(-1, C) # (N * W * H, C)
    out_flat, cache = batchnorm_forward(x_flat, gamma=gamma, beta=beta, bn_param=bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_flat, dgamma, dbeta = batchnorm_backward_alt(dout_flat, cache=cache)
